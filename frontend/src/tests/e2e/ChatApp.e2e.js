...
describe('Chat Application E2E Tests', function() {
  let driver;
  ...
  it('can select a model and send a message', async function() {
    ...
    await driver.wait(until.elementLocated(By.xpath("//li[contains(text(), 'Deepseek')]")), 2000);
    const deepseekOption = await driver.findElement(By.xpath("//li[contains(text(), 'Deepseek')]"));
  ...
  it('handles rapid model switching', async function() {
    ...
    const modelSelect = await driver.findElement(By.css('.MuiSelect-select'));
    const models = ['Deepseek', 'Qwen', 'Gemma'];
